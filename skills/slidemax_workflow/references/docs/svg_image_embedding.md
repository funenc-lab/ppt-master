...
在《设计规范与内容大纲》中定义，每张图片标注**状态**。若图片方案包含「B) 用户提供」，需在 Strategist 阶段完成八项确认后立即运行 `analyze_images`，并在输出设计规范前完成清单填充。
...
5. 导出（可选）
   └── python3 skills/slidemax_workflow/scripts/slidemax.py embed_images *.svg
```
