...
└── slidemax_workflow/  # Workflow-facing skill package
    ...
    └── references/       # Documentation and reference hub
        └── docs/         # Topic-specific reference notes and redirects
...
- `skills/slidemax_workflow/references/docs/`
- 适合新增：skill 元数据、执行约束、资源导航、文档专题说明、兼容跳转入口
...
- `slidemax.subcommands.svg_position`
- 适合新增：命令解析、交互式向导、SVG 分析报告、JSON 配置编排

### SVG 后处理 CLI 扩展

- `slidemax.subcommands.flatten_text`
- `slidemax.subcommands.rounded_rect`
- `slidemax.subcommands.svg_asset`
...
- `slidemax.subcommands.video_generation`
