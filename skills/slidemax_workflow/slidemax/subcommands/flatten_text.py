...
CLI_PROG = "python3 skills/slidemax_workflow/scripts/slidemax.py flatten_tspan"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog='''
When to use:
  - Use this when `tspan`-based text needs to be flattened for stricter SVG compatibility
  - Use interactive mode when you want to choose paths manually

Examples:
  %(prog)s workspace/demo/svg_output
  %(prog)s workspace/demo/svg_output workspace/demo/svg_flat
  %(prog)s --interactive
''',
        add_help=True,
    )
...
def run_cli(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.interactive or not args.input:
        ...
        if not input_path:
            print('Canceled. Usage: python3 skills/slidemax_workflow/scripts/slidemax.py flatten_tspan <input_dir_or_svg> [output_dir]')
            return 0
    else:
